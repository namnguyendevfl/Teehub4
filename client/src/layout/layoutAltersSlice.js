...
const layoutAltersSlice = createSlice({
    name: "layoutAlters",
    initialState,
    reducers: {
        popupChanged(state, action) {
            state.popupSelected = action.payload
        },
    }
})

export default layoutAltersSlice.reducer

export const { 
    popupChanged
} = layoutAltersSlice.actions
